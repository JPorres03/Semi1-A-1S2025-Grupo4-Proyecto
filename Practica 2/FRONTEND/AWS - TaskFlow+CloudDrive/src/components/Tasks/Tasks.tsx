...
function Tasks() {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        setLoading(true);
        const response = await fetch(`http://taskflow-lb-1600142284.us-east-1.elb.amazonaws.com:3001/api/task/${userId}`);
  ...
  const handleAddTask = async (event?: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`http://taskflow-lb-1600142284.us-east-1.elb.amazonaws.com:3001/api/task/${userId}/create`, {
        method: 'POST',
        headers: {
  ...
  const handleToggleComplete = async (taskId: number) => {
    try {
      ...
      const response = await fetch(`http://taskflow-lb-1600142284.us-east-1.elb.amazonaws.com:3001/api/task/complete/${taskId}`, {
        method: 'PATCH',
      });
  ...
  const handleUpdateTask = async (taskId: number, currentTitle: string, currentDescription: string) => {
    ...
    if (formValues) {
      try {
        ...
        const response = await fetch(`http://taskflow-lb-1600142284.us-east-1.elb.amazonaws.com:3001/api/task/update/${taskId}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  const handleDeleteTask = async (taskId: number) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        const response = await fetch(`http://taskflow-lb-1600142284.us-east-1.elb.amazonaws.com:3001/api/task/delete/${taskId}`, {
          method: 'DELETE',
        });
